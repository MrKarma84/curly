...
func (m Model) View() string {
	...
	hintText := "Tab · next panel   Ctrl+R · send   Ctrl+↑/↓ · history   i · infer schema (body)   q · quit"
	if m.histIdx >= 0 {
		hintText = fmt.Sprintf("[history %d/%d]   Ctrl+↑ · older   Ctrl+↓ · newer / back to live", m.histIdx+1, m.store.Len())
	}
